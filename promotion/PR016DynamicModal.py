...
def test_promotion(driver):
    ...
    # İleride bu ID'yi bir config dosyasından, veritabanından veya 
    # sayfadaki listeden rastgele seçerek alabiliriz.
    target_course_id = '3662'

    # JavaScript ile seçimi yap ve 'change' olayını tetikle
    # f-string kullanarak değişkeni script içine gömüyoruz.
    driver.execute_script(f"arguments[0].value = '{target_course_id}'; arguments[0].dispatchEvent(new Event('change'));",
                          select_element)
    time.sleep(2)
